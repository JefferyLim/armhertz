...
uint8_t* aes128_encrypt_sw(uint8_t out[16],
                           const uint8_t in[16],
                           const uint8_t key[16])
{
    ...
    uint8_t state[16];
    uint8_t roundkey[176]; // 11 round keys × 16 bytes
    int i;
...
/*
    ...
    uint8_t rcon = 1;
    for (i = 16; i < 176; i += 16) {
        uint8_t t0 = roundkey[i-4];
        uint8_t t1 = roundkey[i-3];
        uint8_t t2 = roundkey[i-2];
        uint8_t t3 = roundkey[i-1];
        ...
        uint8_t *rk = &roundkey[i];
        uint8_t *rk_prev = &roundkey[i-16];
...
*/
    ...
    for (int round = 1; round <= 9; round++) {
        ...
        state[5] = state[9]; state[9] = state[13];
        state[13] = tmp;
...
static __attribute__((noinline)) int victim(void *varg)
{
    ...
    static uint8_t master_key[16] = {
       ...
       volatile uint8_t ciphertext[16];
    for (;;) {
    

    aes128_encrypt_sw(ciphertext, block, master_key);
