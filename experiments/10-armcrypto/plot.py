...
def f_pool(secret_bit, i, trace, drop_1500s, drop_2200s, drop_2300s, time):
    ...
    if (True):
        ...
        plt.savefig("./plot/aes_%s_%d.png" % (secret_bit, i))
        plt.clf()
        plt.close()
...
def main():
    ...
    out_files = sorted(glob.glob(in_dir + "/aes_*"))
    for f in out_files:
        ...
        if (plot_raw_freq):
            plt.figure(figsize=(20, 3.8))
            plt.plot(freq_trace)
            plt.savefig("./plot/aes_%s_%s.png" % (label, rept_idx))
            plt.clf()
            plt.close()
    ...
    if plot_steady:
        ...
        bins = np.arange(1400, 2600 + 100, 50)    # FIXME: adjust range
